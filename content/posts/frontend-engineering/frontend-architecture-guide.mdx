---
title: "Modern Frontend Architecture: What Actually Works"
date: 2026-03-28
lastmod: 2026-06-07
...
draft: false
summary: >-
  Here is what actually works for modern frontend architecture. I tore down a legacy SPA and rebuilt it as a Server-First application. This guide covers how to organize code, fetch data, and manage scalable frontend systems today.
faqs:
  - question: "What is frontend architecture?"
    answer: >-
      Modern frontend architecture is increasingly server-first. I use React Server Components to keep the heavy JavaScript off the user's device. The browser only handles lightweight interactions.
  - question: "How do I design a scalable frontend architecture?"
    answer: >-
      A scalable frontend separates concerns cleanly. I use a Backend-For-Frontend (BFF) to gather data safely, monorepos to organize code, and keep server data strictly away from local client state.
...
thumbnailAlt: "A definitive guide to frontend system design and Next.js architecture patterns."
---

I still remember the exact commit that broke everything. 

I had just moved a massive React app to the Next.js App Router. I was excited, expecting fast page loads, tiny JavaScript files, and a great developer experience. 
...
Six months later, I checked the Vercel metrics dashboard. My stomach sank.
...
Cleaning up a legacy codebase is exactly like cleaning up my toddler Shree's playroom—you think you're done, and then you find a massive hidden mess in the corner. I had to tear the architecture down and completely start over. 

This post is the result of that tear-down. Learning is rarely a straight line. I want to share the messy middle of getting frontend architecture wrong first. More importantly, I want to share the hard-earned lessons from rebuilding it. After moving to server components, I saw a 65% reduction in bundle size, and the performance benefits were massive.
...
Modern frontend architecture is server-first. Most logic runs on the server using React Server Components, while the browser handles only small interactive parts. This approach improves performance, scalability, and developer experience.

## What is Modern Frontend Architecture?

**What is modern frontend architecture?**  
In modern frontend architecture, the primary goal is moving heavy logic to the server. It is a design style that prioritizes server-rendering to minimize client-side JavaScript. 
...
- **Monorepos** for organizing code safely.
...
## Chapter 2: Edge vs Server Computing

Where you run your code matters just as much as what the code does. This is a massive part of frontend system design.
...
Today, leveraging edge computing allows us to run code closer to the users. But we also learned that you can't push *everything* to the edge.
...
I try to use the Edge for:
- **Authentication Checks:** I check a user's login token before the request even hits the main server. If they aren't logged in, I redirect them to the login page instantly.
- **Rate Limiting:** I use Redis at the edge to block bad actors before they can spam the database.

### When to Use the Origin Server
Heavy lifting still requires a real server. If you try to run a massive database query with five joins on the Edge, it will time out and fail. I keep the Origin server for complex reporting dashboards and streaming large amounts of JSON data.

By splitting the work between Edge speed and Origin power in this production environment, our applications load much faster for everyone.
...
Our Next.js server acts as the BFF. It talks to the microservices, gathers the data, shapes it exactly how the UI needs it, and hands it directly to the components. The user only receives the exact data they need to see. 
...
```
I try to fetch data in parallel now. I also wrap slow charts in React `Suspense` boundaries. This lets the main page load instantly, while the slow charts show a skeleton loader until they finish loading in the background.

---

## Chapter 4: State Management: Redux vs Zustand

This is a harsh truth, but I'll stand by it: **I don't use Redux anymore.**

I spent years writing massive, confusing Redux boilerplates. I wrote reducers, actions, and selectors just to hold database info in the browser's memory. I only did this because fetching data was slow, and I wanted to avoid duplicate requests.
...
*   **Redux:** Forces you to wrap your entire app in a massive context provider. It requires heavy boilerplate. It often leads to storing server data (like a list of users) in the client's memory, which is often an unnecessary layer of complexity.
...
Next.js automatically caches data now. We just fetch data directly inside the Server Component. When a user clicks "Save," we fire a Server Action. Next.js does the work on the server, updates the database, and instantly refreshes the cache. I don't have to write a single line of state management code to sync it. 

I try to enforce a strict separation: if it lives in the database, it is server state. If it is a dropdown menu, it is local `useState`. If it is a temporary multi-step form, it goes in Zustand.

---

## Chapter 5: Monorepo Frontend Architecture for Scalable Applications

In larger projects, code organization can quickly become a nightmare. I've worked on codebases with one giant `components` folder and one giant `hooks` folder, where merge conflicts happened daily.

I've found a structured monorepo architecture solves this.
...
I like to break the app into separate, isolated packages using Turborepo:
...
This fixed messy dependencies. Even better, it sped up testing. When someone edits a button in the UI package, Turborepo knows it only needs to re-test the UI package. The build times dropped significantly.

### Feature Sliced Design Frontend
Inside the main app itself, we adopted **Feature Sliced Design (FSD)**. 

I've started organizing directories strictly by feature (Auth, Checkout, Profile). 

This way, all checkout-related hooks, components, and queries live in the same place, preventing unexpected side effects across other features.
...
I try to pull business logic out of UI components entirely. 
...
Forms used to be the most painful part of my job. Now, I handle most forms with React Hook Form and Zod. 
...
To stop bugs from reaching real users, I changed how I test my code. 

I stopped trying to get 100% test coverage on simple UI wrappers. Writing automated tests just to check if a button has a blue background is rarely a good use of time. Instead, I focus on tests that actually catch real regression bugs:

1. **E2E Testing:** I use Playwright to test our most important user flows, like logging in or checking out. It opens a real browser and clicks through exactly like a human would.
2. **Integration Testing:** I explicitly test server actions and database logic.
3. **Unit Testing:** I reserve unit tests for helper functions, complex math, or decoupled hooks.

---

## Key Takeaways of Modern Frontend Architecture

The way we build frontends today is very different from what we built three years ago. If you want to design a frontend architecture using Next.js, here are the core rules I keep in mind:
...
If you take one thing from this guide, let it be this: **treat client-side JavaScript like a liability, not a default.**

Build less on the client. Let the server do its job. That’s what actually works.
